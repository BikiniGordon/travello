...
namespace Travello.Hubs
{
    ...
    {
        public async Task SendMessage(string chat_room_id, string user_id, string message_text)
        {
            ...
            await Clients.Group(chat_room_id).SendAsync("ReceiveMessage", user_id, message_text);
        }

        // ฟังก์ชันสำหรับดึงผู้ใช้เข้า "กลุ่ม" (เพื่อให้คุยแยกห้องแชทได้)
        public async Task JoinRoom(string chat_room_id)
        {
            await Groups.AddToGroupAsync(Context.ConnectionId, chat_room_id);
        }
    }
}